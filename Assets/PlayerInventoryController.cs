...
public class PlayerInventoryController : Bolt.EntityEventListener<IPlayerState>
{
    ...
    private void WeaponIdChanged() {
        ...
        wizardWeapon = newWep.GetComponent<WizardWeapon>();
    ...
    private void ActiveIdChanged() {
        ...
        if (state.ActiveId >= 0) {
            ...
            activeItem = newActive.GetComponent<WizardActive>();
    ...
    public override void OnEvent(PlayerGotItem pickup) {
        WizardFightItem item = ItemManager.Instance.items[pickup.PickupId];
        if (item.Type == WizardFightItem.ItemType.Weapon)
            state.WeaponId = pickup.PickupId;
        else if (item.Type == WizardFightItem.ItemType.Active)
