...
public partial class BoltWizardWindow : EditorWindow
{
	...
	[NonSerialized]
	Texture2D reviewIcon;

	[NonSerialized]
	GUIContent reviewHeader;

	[NonSerialized]
	GUIContent reviewText;
	...
	private void DrawSupport()
	{
		...
		if (beforeNextCallback == null)
		{
			beforeNextCallback = () =>
			{
				RunCompiler = EditorUtility.DisplayDialog(BoltWizardText.FINISH_TITLE, BoltWizardText.FINISH_QUESTION, "Yes", "No");
				return true;
			};
		}
	}
